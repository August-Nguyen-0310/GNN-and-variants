...
def smart_feature_engineering(df):
    ...
    # Xử lý các giá trị NaN có thể được tạo ra trong quá trình feature engineering
    df = df.fillna(0)
...
X_df = smart_feature_engineering(X_df)
    
# Chuẩn hóa dữ liệu bằng StandardScaler
print("Đang sử dụng StandardScaler để chuẩn hóa dữ liệu...")
...
# --- 2. GATv2 MODEL ARCHITECTURE ---
class GATv2(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        super(GATv2, self).__init__()
        # Sửa lỗi: Khởi tạo mô hình GATv2 đúng
...
# Sửa lỗi: Khởi tạo mô hình GATv2 đúng
model = GATv2(in_channels=data.num_node_features, hidden_channels=64, out_channels=1).to(device)
data = data.to(device)
...
print("\nStarting GATv2 model training with Early Stopping...")
